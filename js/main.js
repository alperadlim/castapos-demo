...
function renderFavoritesPage(){
  ...
  root.innerHTML=items.map(({entry,p})=>{
    ...
    return `<article class="favorite-product-card"><a class="favorite-image" href="${detailHref}"><img src="${p.image}" alt="${p.name}"></a><div class="favorite-copy"><span>${p.brand}</span><h2><a href="${detailHref}">${p.name}</a></h2><div class="favorite-period">${period} Aylık Plan</div><div class="favorite-price"><b>${formatPrice(monthly)}</b><span>/ Aylık</span><strong>${formatPrice(daily)}</strong><small>/ Günlük</small></div><div class="favorite-actions"><button type="button" data-favorite-remove="${p.id}">Listeden Çıkar</button></div></div></article>`;
  }).join('');
}
...
function renderCartDrawer(){
  ...
  const rows=items.map((it,idx)=>{
    ...
    return `<article class="drawer-item"><img src="${p.image}" alt="${p.name}"><div><b>${p.name}</b><span>${period} aylık kiralama · ${qty} adet</span><small>Aylık ödeme</small><strong>${formatPrice(monthly)}</strong></div><button class="remove-line" type="button" data-remove-cart="${idx}">Sil</button></article>`;
  }).join('');
...
function renderEasySteps(){
  ...
  const items=[
    ...
    {icon:'📦', title:'Adres ve kiralama süreni belirle', text:'3, 6 veya 9 ay seçeneklerinden sana uygun planı seç.'},
...
function categoryTypeConfig(label){
  let title='Ürün Tipi';
  if(['Ev Aletleri','Güzellik Teknolojileri','Elektronik'].includes(label)) title='Alt Kategori';
...
initCommon(); initHome(); initCategory(); initProductDetail(); initCart(); renderCartDrawer(); initSupportWidget(); renderFavoritesPage(); updateFavoriteButtons();
