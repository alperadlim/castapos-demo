...
function openCartDrawer(){if(window.matchMedia('(max-width:760px)').matches){document.querySelector('[data-cart-drawer]')?.classList.remove('open');document.body.classList.remove('cart-open');showToast('Ürün sepetine eklendi.');return;}document.querySelector('[data-cart-drawer]')?.classList.add('open');document.body.classList.add('cart-open')}
...
(function(){
  ...
  initProductDetail=function(){
    ...
    function openLightbox(){
      ...
      if(!lb){
        document.body.insertAdjacentHTML('beforeend',`<div class="product-lightbox"><div class="product-lightbox-dialog"><div class="product-lightbox-image"><button type="button" class="product-lightbox-close" aria-label="Kapat">×</button><button type="button" class="product-lightbox-arrow prev" aria-label="Önceki görsel">‹</button><img src="" alt=""><button type="button" class="product-lightbox-arrow next" aria-label="Sonraki görsel">›</button></div><div class="product-lightbox-thumbs"></div></div></div>`);
        lb=document.querySelector('.product-lightbox');
        const dialog=lb.querySelector('.product-lightbox-dialog');
        lb.addEventListener('click',e=>{ if(e.target===lb || e.target===dialog) lb.classList.remove('open'); });
...
/* === V53 interaction refinements === */
(function(){
  const clamp=(value,min,max)=>Math.min(max,Math.max(min,value));

  function installStableMobileFavoriteIcon(){
    const apply=()=>{
      const icon=document.querySelector('[data-mobile-bottom-nav] a[href="favorilerim.html"] > span');
      if(!icon || icon.classList.contains('mobile-favorite-icon')) return;
      icon.className='mobile-favorite-icon';
      icon.innerHTML='<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78Z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    };
    apply();
  ...
  function installLightboxZoom(){
    ...
    const prepare=()=>{
      ...
      const viewport=lightbox?.querySelector('.product-lightbox-image');
      const image=viewport?.querySelector('img');
      if(!lightbox || !viewport || !image || viewport.dataset.zoomReady==='1') return;
      viewport.dataset.zoomReady='1';
      viewport.insertAdjacentHTML('afterend','<div class="lightbox-zoom-controls"><button type="button" data-zoom-out aria-label="Uzaklaştır">−</button><button type="button" data-zoom-reset>100%</button><button type="button" data-zoom-in aria-label="Yakınlaştır">+</button></div>');
      const controls=viewport.parentElement.querySelector('.lightbox-zoom-controls');
      const pointers=new Map();
      let scale=1, x=0, y=0, startScale=1, startDistance=0, dragStart=null;
      const render=(animate=true)=>{
        viewport.classList.toggle('is-interacting',!animate);
      ...
      controls.addEventListener('click',event=>{
        ...
        if(event.target.closest('[data-zoom-in]')) setScale(scale+.35);
        if(event.target.closest('[data-zoom-out]')) setScale(scale-.35);
        if(event.target.closest('[data-zoom-reset]')) reset();
      });
      ...
      viewport.addEventListener('pointerdown',event=>{
        if(event.target.closest('.product-lightbox-close, .product-lightbox-arrow')) return;
        if(event.pointerType==='mouse' && scale<=1.02) return;
        viewport.setPointerCapture?.(event.pointerId);
        pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});
        viewport.classList.add('is-dragging','is-interacting');
        if(pointers.size===2){startDistance=distance();startScale=scale;dragStart=null;}
        else if(pointers.size===1 && scale>1){dragStart={px:event.clientX,py:event.clientY,x,y};}
      });
      ...
      const release=event=>{
        ...
        if(pointers.size<2){startDistance=0;startScale=scale;}
        if(!pointers.size){dragStart=null;viewport.classList.remove('is-dragging','is-interacting');render();}
      };
  ...
  function repairCompareClose(){
    const hiddenKey='castaposCompareHidden';
    const setHidden=value=>{try{value?sessionStorage.setItem(hiddenKey,'1'):sessionStorage.removeItem(hiddenKey)}catch(e){}};
    const isHidden=()=>{try{return sessionStorage.getItem(hiddenKey)==='1'}catch(e){return false}};
    if(typeof updateCompareUI==='function'){
      const previous=updateCompareUI;
      updateCompareUI=function(){
        previous();
        const bar=document.querySelector('[data-compare-bar]');
        const items=typeof compareItems==='function'?compareItems():[];
        if(!items.length || isHidden()) bar?.classList.remove('show');
      };
    }
    document.addEventListener('click',event=>{
      const toggle=event.target.closest('[data-compare-toggle]');
      if(toggle) setHidden(false);
      const hide=event.target.closest('[data-compare-hide]');
      if(hide){
        event.preventDefault();
        event.stopImmediatePropagation();
        setHidden(true);
        document.querySelector('[data-compare-bar]')?.classList.remove('show');
        return;
      }
      const close=event.target.closest('[data-compare-close]');
      if(close){
        event.preventDefault();
        event.stopImmediatePropagation();
        document.querySelector('[data-compare-modal]')?.classList.remove('open');
      }
    },true);
    try{updateCompareUI?.();}catch(e){}
  }

  installStableMobileFavoriteIcon();
  installLightboxZoom();
  repairCompareClose();
})();


/* V57 stable square product lightbox controls */
(function(){
  const absoluteSrc=src=>{try{return new URL(src,location.href).href}catch(e){return src||''}};

  function resetViewer(lightbox){
    const viewport=lightbox?.querySelector('.product-lightbox-image');
    const image=viewport?.querySelector(':scope > img');
    if(image) image.style.transform='translate3d(0px,0px,0) scale(1)';
    viewport?.classList.remove('is-dragging','is-interacting');
    lightbox?.classList.remove('is-zoomed');
    const reset=lightbox?.querySelector('[data-zoom-reset]');
  ...
  function arrangeViewer(){
    ...
    const prev=lightbox.querySelector('.product-lightbox-arrow.prev');
    const next=lightbox.querySelector('.product-lightbox-arrow.next');
    const close=lightbox.querySelector('.product-lightbox-close');
    [prev,next,close].forEach(button=>{if(button && button.parentElement!==viewport) viewport.appendChild(button)});
  }

  function gallerySources(lightbox){
    return [...lightbox.querySelectorAll('.product-lightbox-thumbs img')]
      .map(img=>img.getAttribute('src'))
      .filter(Boolean);
  }

  function navigate(lightbox,direction){
    const image=lightbox.querySelector('.product-lightbox-image > img');
    const sources=gallerySources(lightbox);
    if(!image || !sources.length) return;
    const current=absoluteSrc(image.getAttribute('src'));
    let index=sources.findIndex(src=>absoluteSrc(src)===current);
    if(index<0) index=0;
    index=(index+direction+sources.length)%sources.length;
    image.src=sources[index];
    lightbox.querySelectorAll('.product-lightbox-thumbs button').forEach((button,i)=>button.classList.toggle('active',i===index));
    resetViewer(lightbox);
  }

  document.addEventListener('click',event=>{
    const lightbox=event.target.closest('.product-lightbox');
    if(!lightbox) return;
    const close=event.target.closest('.product-lightbox-close');
    const prev=event.target.closest('.product-lightbox-arrow.prev');
    const next=event.target.closest('.product-lightbox-arrow.next');
    if(close){
      event.preventDefault();
      event.stopImmediatePropagation();
      resetViewer(lightbox);
    ...
    if(prev || next){
      event.preventDefault();
      event.stopImmediatePropagation();
      navigate(lightbox,prev?-1:1);
    }
  },true);

  document.addEventListener('keydown',event=>{
    const lightbox=document.querySelector('.product-lightbox.open');
    if(!lightbox) return;
    if(event.key==='Escape'){
      event.preventDefault();
      resetViewer(lightbox);
  ...
  const observer=new MutationObserver(()=>arrangeViewer());
  observer.observe(document.body,{childList:true,subtree:true});
  arrangeViewer();
})();

/* V60 mobile lightbox blank-area close */
(function(){
  document.addEventListener('click',event=>{
    const lightbox=document.querySelector('.product-lightbox.open');
    if(!lightbox) return;
    if(event.target.closest('.product-lightbox-image, .lightbox-zoom-controls')) return;
    lightbox.classList.remove('open','is-zoomed');
    const image=lightbox.querySelector('.product-lightbox-image > img');
    if(image) image.style.transform='translate3d(0px,0px,0) scale(1)';
  },true);
})();
